...
const GetProfile = ({ profile }) => {
    ...
    return (
        <div className='w-full flex justify-between bg-white/10 border border-gray-800 p-10 rounded-xl'>
            <div className='w-full flex flex-col md:flex-row items-center  justify-evenly'>
                ...
                <div className='w-[50%] text-center h-56 flex flex-col gap-4 items-center justify-center p-4'>
                    <div>
                        <p className='text-4xl font-semibold'>@{profile.username}</p>
                        <p className='text-gray-300 text-lg'>{profile.name}</p>
                        <p className='text-gray-400'>{profile.bio}</p>
                    </div>
                    <div className='flex w-fit md:w-full justify-evenly gap-4 text-blue-500 bg-white/10 rounded-lg p-4' onClick={() => changeFollowTab()}>
                        <div>
